document.addEventListener('DOMContentLoaded', function () {
    ...
    if (toggleTeamButton && teamList) {
        toggleTeamButton.addEventListener('click', function () {
            teamList.style.display = teamList.style.display === 'none' || teamList.style.display === '' ? 'block' : 'none';
        });
    }
    ...
    if (searchInput && jobList) {
        searchInput.addEventListener('input', function () {
            ...
            for (let i = 0; i < jobs.length; i++) {
                const jobTitle = jobs[i].textContent.toLowerCase();
                jobs[i].style.display = jobTitle.includes(filter) ? '' : 'none';
            }
        });
    }
});
